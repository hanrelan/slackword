defmodule Slackword.Crossword.Parsers.XmlParser do
  ...
  defp parse_clues(crossword, xml) do
    ...
    clues = xml |> xpath(~x"//clue"l, word_id: ~x"./@word"s, number: ~x"./@number"s, format: ~x"./@format"s, text: ~x"./text()"ls)
                |> Stream.map(fn clue_map -> Map.put(clue_map, :text, Enum.join(Map.get(clue_map, :text, []), "")) end)
